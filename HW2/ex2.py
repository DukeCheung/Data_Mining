...
def gradient_descent(a, b, c, data, learning_rate):
    ...
    for i in range(0, length):
        ...
        y = distance[i]
        z = price[i]
        gradient_a += learning_rate * (1/float(length)) * (((a * x) + (b * y) + c) - z) * x
        gradient_b += learning_rate * (1/float(length)) * (((a * x) + (b * y) + c) - z) * y 
        gradient_c += learning_rate * (1/float(length)) * (((a * x) + (b * y) + c) - z)
    next_a = a - gradient_a
    next_b = b - gradient_b
    next_c = c - gradient_c
...
plt.ylabel('Error')
plt.show()
print(errorForTraining, errorForTesting)
